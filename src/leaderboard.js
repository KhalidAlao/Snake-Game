...
function clearLeaderboard(){entries=[];localStorage.removeItem("leaderboard");if(onChange) onChange(entries);}
function qualifiesForLeaderboard(score){const current=getEntries();return current.length<5||score>current[current.length-1].score;}
function setLeaderboardChangeCallback(cb){onChange=cb;}

export { addOrUpdateEntry, getEntries, clearLeaderboard, qualifiesForLeaderboard, setLeaderboardChangeCallback };
